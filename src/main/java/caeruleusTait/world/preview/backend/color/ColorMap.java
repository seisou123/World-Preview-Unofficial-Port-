...
public class ColorMap {
    ...
    /**
     * Calculates the color for a given position with controllable interpolation.
     *
     * @param position must be between 0 and 1 inclusive
     * @param labBlend 0.0 = pure sRGB lerp (hard transitions), 1.0 = pure Lab
     *                 lerp (perceptually smooth), in between = proportional mix
     */
    public Color getBlended(float position, float labBlend) {
        if (position < 0) {
            return colors[0];
        } else if (position > 1) {
            return colors[colors.length - 1];
            ...
            return lerpBlended(colors[floor], colors[floor + 1], pos - floor, labBlend);
        }
    }

    /**
     * Returns an ARGB color for the given position with controllable interpolation.
     *
     * @param position must be between 0 and 1 inclusive
     * @param labBlend 0.0 = pure sRGB lerp, 1.0 = pure Lab lerp
     */
    public int getARGBBlended(float position, float labBlend) {
        final Color c = getBlended(position, labBlend);
        final int R = ((int) (c.r * 255f)) & 0xFF;
        final int G = ((int) (c.g * 255f)) & 0xFF;
        final int B = ((int) (c.b * 255f)) & 0xFF;
        return (0xFF << 24) | (R << 16) | (G << 8) | B;
    ...
    /**
     * Interpolate between two colors with a controllable blend of sRGB and
     * L*ab space.
     * <p>
     * When {@code labBlend = 0}, pure sRGB channel-wise lerp is used, producing
     * harder, more direct transitions. When {@code labBlend = 1}, full L*ab
     * interpolation is used, producing perceptually smooth gradients. Values
     * in between proportionally mix the two results.
     *
     * @param lower    Lower color
     * @param upper    Upper color
     * @param amount   Interpolation factor (0 = lower, 1 = upper)
     * @param labBlend 0 = sRGB lerp, 1 = Lab lerp, between = mix
     * @return Interpolated color
     */
    public static Color lerpBlended(final Color lower, final Color upper, final float amount, final float labBlend) {
        if (labBlend >= 1.0f) {
            return lerp(lower, upper, amount);
        }
        if (labBlend <= 0.0f) {
            // Pure sRGB lerp — direct, no perceptual warping
            return new Color(
                    clamp(lerp(upper.r, lower.r, amount), 0f, 1f),
                    clamp(lerp(upper.g, lower.g, amount), 0f, 1f),
                    clamp(lerp(upper.b, lower.b, amount), 0f, 1f)
            );
        }
        // Mix: compute both and blend
        final Color rgbResult = new Color(
                clamp(lerp(upper.r, lower.r, amount), 0f, 1f),
                clamp(lerp(upper.g, lower.g, amount), 0f, 1f),
                clamp(lerp(upper.b, lower.b, amount), 0f, 1f)
        );
        final Color labResult = lerp(lower, upper, amount);
        return new Color(
                clamp(lerp(labResult.r, rgbResult.r, labBlend), 0f, 1f),
                clamp(lerp(labResult.g, rgbResult.g, labBlend), 0f, 1f),
                clamp(lerp(labResult.b, rgbResult.b, labBlend), 0f, 1f)
        );
    }
