...
public class ColorChooser extends AbstractWidget {
    ...
    public void extractWidgetRenderState(GuiGraphicsExtractor guiGraphicsExtractor, int i, int j, float f) {
        // render background
        guiGraphicsExtractor.fill(getX() - 2, getY() - 2, getX() + width + 2, getY() + height + 2, 0x77000000);

        // Render saturation value chooser using horizontal strips
        int leftX = getX();
        int topY = getY();
        ...
        int strips = 32;
        int stripWidth = (svSquareSize + strips - 1) / strips;
        for (int s = 0; s < strips; s++) {
            int stripLeft = leftX + s * svSquareSize / strips;
            int stripRight = leftX + (s + 1) * svSquareSize / strips;
            if (stripRight <= stripLeft) continue;
            float sat = (float)(s + 0.5) / strips;
            int topColor = Color.HSBtoRGB(hue, sat, 1.0f);
            int botColor = Color.HSBtoRGB(hue, sat, 0.0f);
            guiGraphicsExtractor.fillGradient(stripLeft, topY, stripRight, botY, topColor, botColor);
        }
        ...
        guiGraphicsExtractor.fill(satX - 4, valY - 4, satX + 4, valY + 4, value > .3 ? 0xFF000000 : 0xFFFFFFFF);
        guiGraphicsExtractor.fill(satX - 3, valY - 3, satX + 3, valY + 3, argbColor);
        ...
        for (int s = 0; s < hueStrips; s++) {
            ...
            guiGraphicsExtractor.fill(leftX, stripTop, rightX, stripBottom, Color.HSBtoRGB(stripHue, 1f, 1f));
        }

        // Render hue indicator
        int hueY = topY + Math.round((1f - hue) * svSquareSize);
        guiGraphicsExtractor.fill(leftX - 2, hueY - 4, rightX + 2, hueY + 4, 0xFF000000);
        guiGraphicsExtractor.fill(leftX - 1, hueY - 3, rightX + 1, hueY + 3, argbHueOnly);

        // Render final color box
        guiGraphicsExtractor.fill(getX(), botY + SEPARATOR, getX() + width, getY() + height, argbColor);
    }
    ...
    public boolean mouseEvent(double mouseX, double mouseY, int button, boolean playSound) {
        ...
        if (Minecraft.getInstance().gui.screen() != null) {
            Minecraft.getInstance().gui.screen().setFocused(this);
        }
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        return mouseEvent(event.x(), event.y(), event.button(), true);
    }
