...
public final class BiomeSharePanel extends AbstractWidget {
    ...
    protected void extractWidgetRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.text(font, TITLE, getX() + 6, getY() + 6, TEXT);
        ...
        if (insights != null) {
            ...
            graphics.text(font, diversity, getX() + 6, getY() + 20, TEXT_SOFT);
        }
        ...
        if (insights != null) {
            ...
            for (LegendItem item : layoutLegend(font)) {
                ...
                graphics.text(font, item.label(), item.x() + 8, item.y(), TEXT_SOFT);
            }
        }
        ...
        for (int i = scrollOffset; i < rows.size(); i++) {
            ...
            graphics.text(font, row.name(), rowX + 5, rowY + 4, TEXT);
            String share = String.format(Locale.ROOT, "%.1f%%", row.sharePercent());
            int shareX = getX() + width - 6 - font.width(share);
            graphics.text(font, share, shareX, rowY + 4, TEXT_SOFT);
            if (row.stars() > 0) {
                String stars = "★".repeat(row.stars());
                graphics.text(font, stars, shareX - 4 - font.width(stars), rowY + 4, TEXT_STARS);
            }
        }
