...
public class PreviewDisplay extends AbstractWidget implements AutoCloseable {
    ...
    // After invalidateRenderCache(), needsInitialQueue is true and preload is
    // forced to 0 so the range matches queueEarlyPreviewRange().  But on the
    // very next frame, needsInitialQueue becomes false and preload kicks in,
    // producing a DIFFERENT (larger) range.  WorkManager.queueRange sees a
    // mismatch, aborts the in-flight pass, and starts a new one �?which is
    // then aborted again on the next frame when preload flips back to 0
    // (because activeBatchCount > 0).  This infinite abort/restart loop
    // prevents worker threads from ever writing data, causing the
    // "black screen until drag" bug.
    //
    // Fix: keep preload at 0 until worker threads have actually written data
    // for the current session (writeCounter > 0).  Once data is confirmed,
    // preload is safe because the non-preloaded range has already been
    // sampled and the preloaded range only adds surrounding chunks.
...
resizeImage();
    ...
    public void reloadData() {
        ...
        textureNeedsUpload = true;
        // Reset preload oscillation guard for new world data.
    ...
    public void extractWidgetRenderState(GuiGraphicsExtractor guiGraphicsExtractor, int x, int y, float f) {
        ...
        if (renderFrameCount <= 5 || renderFrameCount % 300 == 0) {
            boolean inChildren = minecraft.gui.screen() != null && minecraft.gui.screen().children().contains(this);
        }
        ...
        synchronized (dataProvider) {
            if (dataProvider.setupFailed()) {
                previewImg.fillRect(0, 0, texWidth, texHeight, 0xFF000000);
                previewTexture.upload();
                WorldPreviewClient.renderTexture(guiGraphicsExtractor, previewTexture, xMin, yMin, xMax, yMax);
                ...
                // Mark that we have received initial data, enabling preload in
                // queueGeneration().  See initialDataReceived javadoc for details.
                ...
                if (!needRerender) {
                    ...
                    renderStructures(cachedRenderData, guiGraphicsExtractor);
                    renderPlayerAndSpawn(guiGraphicsExtractor);
                    renderSpawnPin(guiGraphicsExtractor);
                    ...
                    guiGraphicsExtractor.enableScissor(xMin, yMin, xMax, yMax);
                    renderStructures(renderData, guiGraphicsExtractor);
                    renderPlayerAndSpawn(guiGraphicsExtractor);
                    renderSpawnPin(guiGraphicsExtractor);
                    guiGraphicsExtractor.disableScissor();
                ...
                double mouseZ = (minecraft.mouseHandler.ypos() * minecraft.getWindow().getGuiScaledHeight()) / minecraft.getWindow()
                        .getScreenHeight();
                updateTooltip(guiGraphicsExtractor, mouseX, mouseZ);
            }
        }
        ...
        if (config.showStatistics) {
            dataVisualizer.renderStatistics(guiGraphicsExtractor, xMin, yMin, xMax, yMax, visibleBiomes, visibleStructures);
        }
    }
    ...
    private void queueGeneration() {
        ...
        if (config.enablePreload && !needsInitialQueue && initialDataReceived) {
            ...
            // NOTE: preload is ONLY enabled after initialDataReceived is true,
            // i.e. after worker threads have written at least some data for the
            // current session.  Before that, using preload would produce a
            // different (larger) range than the initial queue, causing an
            // infinite abort/restart loop in WorkManager �?a major cause of the "black screen until drag" bug.
    ...
    private void renderStructures(List<RenderHelper> renderData, GuiGraphicsExtractor guiGraphicsExtractor) {
    ...
    private void renderSpawnPin(GuiGraphicsExtractor guiGraphicsExtractor) {
        if (spawnPinPos == null) return;
        renderStickyIcon(guiGraphicsExtractor, playerIcon, spawnPinPos);
    }

    private void renderPlayerAndSpawn(GuiGraphicsExtractor guiGraphicsExtractor) {
    ...
    private void updateTooltip(GuiGraphicsExtractor guiGraphicsExtractor, double mouseX, double mouseY) {
    ...
    public void onClick(MouseButtonEvent event, boolean doubleClick) {
        ...
        if (minecraft.gui.screen() != null) {
            minecraft.gui.screen().setFocused(this);
        }
    ...
    public void invalidateRenderCache() {
        ...
        needsInitialQueue = true;
        textureNeedsUpload = true;
        // Reset preload oscillation guard: preload must stay at 0 until worker
        // threads write data for the new session.
        initialDataReceived = false;
