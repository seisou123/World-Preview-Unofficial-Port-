...
public final class AnalysisPanel extends AbstractWidget {
    ...
    protected void renderWidget(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.drawString(Minecraft.getInstance().font, Component.translatable("world_preview.analysis.metrics"), textX, textY, 0xFFFFFFFF);
        if (metrics == null) {
    ...
    private int drawSpawnSection(GuiGraphics graphics, int textX, int y) {
        if (spawnScore == null && spawnReasons.isEmpty()) {
            return y;
        }
        graphics.drawString(Minecraft.getInstance().font, WorldPreviewComponents.ANALYSIS_SPAWN_SCORE, textX, y, 0xFFFFFFFF);
        int line = y + 12;
        if (spawnScore != null) {
    ...
    private int drawTopBiomesSection(GuiGraphics graphics, int textX, int y) {
        if (topBiomes.isEmpty()) {
            return y;
        }
        graphics.drawString(Minecraft.getInstance().font, WorldPreviewComponents.ANALYSIS_TOP_BIOMES, textX, y, 0xFFFFFFFF);
        int line = y + 12;
    ...
    private void drawLine(GuiGraphics graphics, int x, int y, Component text) {
