...
public class BiomesList extends BaseObjectSelectionList<BiomesList.BiomeEntry> {
    ...
    public void extractWidgetRenderState(GuiGraphicsExtractor guiGraphicsExtractor, int mouseX, int mouseY, float partialTick) {
        ...
        if (searchActive && searchStatus != null) {
            int statusY = getY() + 2;
            guiGraphicsExtractor.fill(getX(), statusY - 1, getX() + getWidth(), statusY + minecraft.font.lineHeight + 1, 0xAA000000);
            guiGraphicsExtractor.text(minecraft.font, searchStatus, getX() + 4, statusY, 0xFFFFFF00);
        }
    }
    ...
    public class BiomeEntry extends BaseObjectSelectionList.Entry<BiomeEntry> {
        ...
        public @NotNull Component getNarration() {
            return Component.translatable("narrator.select", this.name);
                ...
                @Override
        public void extractContent(@NotNull GuiGraphicsExtractor guiGraphicsExtractor, int mouseX, int mouseY, boolean hovered, float partialTick) {
            int top = getContentY();
            int left = getContentX();
            guiGraphicsExtractor.fill(left + 3, top + 1, left + 13, top + 11, nativeColor(color));
            ...
            guiGraphicsExtractor.text(BiomesList.this.minecraft.font, formatName, left + 16, top + 2, nameColor);
            ...
            if (previewContainer.worldPreview().cfg().showBiomeCounts) {
                ...
                guiGraphicsExtractor.text(BiomesList.this.minecraft.font, countStr, rowRight - countWidth - 4, top + 2, countColor);
            }
        }
