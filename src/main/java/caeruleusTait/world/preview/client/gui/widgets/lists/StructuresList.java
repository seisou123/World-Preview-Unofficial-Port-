...
public class StructuresList extends BaseObjectSelectionList<StructuresList.StructureEntry> {
    ...
    public class StructureEntry extends BaseObjectSelectionList.Entry<StructuresList.StructureEntry> implements StructureRenderInfo {
        ...
        public void extractContent(
            ...
            if (item != null) {
                guiGraphicsExtractor.item(itemStack, xMin, yMin);
            } else if (iconTexture != null && !textureClosed) {
                WorldPreviewClient.renderTexture(guiGraphicsExtractor, iconTexture, xMin, yMin, xMax, yMax);
            }
            String formatName = renderedName();
            guiGraphicsExtractor.text(minecraft.font, formatName, left + 16 + 4, top + 6, 0xFFFFFFFF);
            toggleVisible.setPosition(getRowRight() - 22, top);
            toggleVisible.extractRenderState(guiGraphicsExtractor, mouseX, mouseY, partialTick);
        }
