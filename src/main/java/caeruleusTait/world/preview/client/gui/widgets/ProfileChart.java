    ...
    protected void renderWidget(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (sampled.size() >= 2) {
            ...
            for (int i = 1; i < points.size(); i++) {
                ...
                int y1 = yFor(a.height(), min, max, top, bottom);
                int y2 = yFor(b.height(), min, max, top, bottom);
                graphics.fill(Math.min(x1, x2), y1, Math.max(x1, x2) + 1, y1 + 1, 0xFF63B3ED);
                graphics.fill(x2, Math.min(y1, y2), x2 + 1, Math.max(y1, y2) + 1, 0xFF63B3ED);
            ...
            if (mouseX >= left && mouseX <= right && mouseY >= top && mouseY <= bottom) {
                ...
                graphics.drawString(Minecraft.getInstance().font, Component.literal(hoverText), tooltipX, tooltipY, 0xFFFFFFFF);
            }
        } else {
            hoverText = "";
