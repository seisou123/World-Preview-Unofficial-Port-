...
final class PreviewDataVisualizer {
    ...
    void renderMinimap(GuiGraphics guiGraphics, int xMin, int yMin, int xMax, int yMax, BlockPos centerPos) {
        ...
        guiGraphics.fill(miniX - 2, miniY - 2, miniX + miniW + 2, miniY + miniH + 2, 0xDD0A0A12);
        guiGraphics.fill(miniX - 1, miniY - 1, miniX + miniW + 1, miniY + miniH + 1, 0xFF1a1a2e);
        WorldPreviewClient.renderTexture(guiGraphics, minimapTexture, miniX, miniY, miniX + miniW, miniY + miniH);
        guiGraphics.fill(miniX, miniY, miniX + miniW, miniY + 1, 0x33FFFFFF);
        ...
        if (bvX2 > bvX && bvY2 > bvY) {
            guiGraphics.fill(bvX, bvY, bvX2, bvY + 1, vpColor);
            guiGraphics.fill(bvX, bvY2 - 1, bvX2, bvY2, vpColor);
            guiGraphics.fill(bvX, bvY, bvX + 1, bvY2, vpColor);
            guiGraphics.fill(bvX2 - 1, bvY, bvX2, bvY2, vpColor);
        }
        ...
        guiGraphics.fill(crossX - 2, crossY, crossX + 3, crossY + 1, crossColor);
        guiGraphics.fill(crossX, crossY - 2, crossX + 1, crossY + 3, crossColor);
        ...
        if (labelY >= yMin) {
            guiGraphics.fill(labelX - 2, labelY - 1, labelX + labelW + 2, labelY + minecraft.font.lineHeight, 0x80000000);
            guiGraphics.drawString(minecraft.font, label, labelX, labelY, 0xBBFFFFFF);
        }
    }

    void renderStatistics(GuiGraphics guiGraphics, int xMin, int yMin, int xMax, int yMax,
        ...
        guiGraphics.fill(startX - 3, startY - 1, xMax - 2, startY + minecraft.font.lineHeight, 0x80000000);
        guiGraphics.drawString(minecraft.font, stats, startX, startY, 0xFFFFFFFF);
    }
}
