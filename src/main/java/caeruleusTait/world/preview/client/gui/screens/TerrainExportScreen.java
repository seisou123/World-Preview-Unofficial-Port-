 ...
 * Adapted for the MC 26.1.2 GuiGraphicsExtractor API.
 */
public final class TerrainExportScreen extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.centeredText(font, WorldPreviewComponents.TERRAIN_EXPORT_TITLE, width / 2, 8, SettingsTheme.TEXT);
    ...
    private void renderProgressBar(GuiGraphicsExtractor graphics) {
        int x = MX;
        ...
        if (status.outputPath() != null) {
            ...
            for (int i = 0; i < path.length(); i++) {
                ...
                if (drawX + lineW + charW > x + screenMaxW && line.length() > 0) {
                    graphics.text(font, Component.literal(line.toString()), drawX, drawY, SettingsTheme.TEXT_DIM);
                    drawY += 12;
                    line.setLength(0);
                    lineW = 0;
                }
    ...
    private void renderLegendColors(GuiGraphicsExtractor graphics) {
        int x = MX;
        int y = exportContentStartY() + SH + GAP + 4 + 16 + 14 * 4 + 8;

        Component legendTitle = WorldPreviewComponents.TERRAIN_EXPORT_LEGEND;
        graphics.text(font, legendTitle, x, y, SettingsTheme.TEXT);
        ...
        for (int i = 0; i < cats.length; i++) {
            ...
            graphics.text(font, catLabel, curX + colorSize + colorTextGap, itemY, SettingsTheme.TEXT);
